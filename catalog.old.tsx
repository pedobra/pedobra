...
const SPECIALTY_OPTIONS = [
    'Constru├º├úo Civil',
    'El├®trica',
    'Hidr├íulica / Encanamento',
    'Alvenaria / Estrutura',
    'Acabamento / Pintura',
    'Esquadrias / Vidra├ºaria',
    'Impermeabiliza├º├úo',
    'Ar Condicionado / HVAC',
    'Seguran├ºa / CFTV',
    'Funda├º├úo / Terraplanagem',
    'Paisagismo',
    'Loca├º├úo de Equipamentos',
    'Distribui├º├úo de Materiais',
...
const AdminCatalog = () => {
    ...
    const downloadCSVTemplate = () => {
        ...
        const rows = [
            ...
            "Cabo Flex├¡vel 2.5mm;El├®trica;rolo",
            "Tubo PVC 100mm;Hidr├íulica;m",
            "Tinta Acr├¡lica Branca;Acabamento;lata"
        ].join("\n");
    ...
    const handleCSVImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            if (materialsToImport.length === 0) {
                if (skippedCount > 0) {
                    alert(`Todos os ${skippedCount} materiais do arquivo j├í est├úo cadastrados.`);
                } else {
                    alert('Nenhum material v├ílido encontrado no arquivo ou formato incorreto.');
                }
                e.target.value = '';
                return;
            }
            ...
            const confirmMsg = skippedCount > 0
                ? `Deseja importar ${materialsToImport.length} novos materiais? (${skippedCount} duplicados ser├úo ignorados).`
            ...
            if (error) {
                alert('Erro na importa├º├úo: ' + error.message);
            } else {
    ...
    return (
        <div className="catalog-view">
            <header className="view-header">
                <div className="header-info">
                    ...
                    <p className="page-subtitle">Centraliza├º├úo de dados para otimiza├º├úo de suprimentos.</p>
                </div>
                    ...
                    <thead>
                        {type === 'materials' ? (
                            <tr>
                                ...
                                <th style={{ textAlign: 'right' }}>A├ç├òES</th>
                            </tr>
                        ) : (
                            <tr>
                                <th>FORNECEDOR</th>
                                    ...
                                    <>
                                                ...
                                                <div>
                                                    ...
                                                    {(item.address_city || item.address_state) && (
                                                        <div className="supplier-location">
                                                            <MapPin size={10} />
                                                            {[item.address_city, item.address_state].filter(Boolean).join(' ÔÇô ')}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        <td style={{ maxWidth: 130 }}>
                                            {item.specialty
                                                ...
                                                : <span className="text-muted-sm">ÔÇö</span>
                                            }
                                        </td>
                                        <td>
                                        ...
                                        <td>{item.address_neighborhood || <span className="text-muted-sm">ÔÇö</span>}</td>
                                        <td style={{ textAlign: 'center' }}>
                                            {item.delivers
                                                ? <span className="delivers-badge yes"><Truck size={12} /></span>
                                                : <span className="delivers-badge no">ÔÇö</span>
                                            }
                                        </td>
                                    </>
                                )}
                ...
                <div className="modal-overlay glass" onClick={() => setShowModal(false)}>
                    <div className="modal-card premium-card animate-fade" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2 className="modal-title">{editingItem ? 'Editar Registro' : 'Novo Registro no Cat├ílogo'}</h2>
                            <p>Insira os detalhes t├®cnicos.</p>
                        </div>
                        <form onSubmit={handleSubmit} className="modal-form">
                            {type === 'materials' ? (
                                <>
                                    <div className="input-field">
                                        <label>Designa├º├úo do Insumo</label>
                                    ...
                                    <div className="input-group">
                                        <div className="input-field">
                                            ...
                                            <select value={materialForm.category} onChange={e => setMaterialForm({ ...materialForm, category: e.target.value })} required>
                                                ...
                                                <option value="Hidr├íulica">Hidr├íulica</option>
                                                <option value="Acabamento">Acabamento</option>
                                                <option value="Outros">Outros</option>
                                            </select>
                                        </div>
                                        <div className="input-field">
                                            <label>Unidade Padr├úo</label>
                                            <select value={materialForm.unit} onChange={e => setMaterialForm({ ...materialForm, unit: e.target.value })} required>
                                                ...
                                                <option value="gal├úo">gal├úo (Gal├úo)</option>
                                ...
                                <>
                                    {/* ÔöÇÔöÇ Identifica├º├úo ÔöÇÔöÇ */}
                                    <div className="form-section-label">Identifica├º├úo</div>
                                    <div className="input-field">
                                        <label>Raz├úo Social / Nome Fantasia</label>
                                    ...
                                    <div className="input-group">
                                        ...
                                        <div className="input-field">
                                            <label>N├¡vel de Confiabilidade (1-5)</label>
                                    ...
                                    <div className="input-group" style={{ alignItems: 'flex-start' }}>
                                        ...
                                        <div className="input-field">
                                            ...
                                            >
                                                ...
                                                <span className="toggle-label">{delivers ? 'Sim' : 'N├úo'}</span>
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="input-group" style={{ gridTemplateColumns: '2fr 1fr' }}>
                                        <div className="input-field">
                                            <label>Cidade</label>
                                            <input type="text" value={supplierForm.address_city} onChange={e => setSupplierForm(p => ({ ...p, address_city: e.target.value }))} placeholder="S├úo Paulo" />
                                        </div>
                                        <div className="input-field">
                            ...
                            <div className="modal-actions-btns">
                                ...
                                <button type="submit" className="btn-primary" disabled={loading}>
                                    {loading ? 'Sincronizando...' : (editingItem ? 'Confirmar Edi├º├úo' : 'Confirmar Registro')}
                                </button>
                            </div>
                        </form>
                    </div>
        ...
        /* Column widths ÔÇö 6 cols: FORNECEDOR | ESPECIALIDADE | CONTATO | BAIRRO | ENTREGA | A├ç├òES */
